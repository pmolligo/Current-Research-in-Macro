...
\vspace{8pt}
	\begin{itemize}
	 ...
	 $y_{1}^{s}=y_{1}^{d}+\textcolor{red}{\bar m}\hat y_{1}$, $\bar m \in [0, 1]$
		\vspace{10pt}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Iteration gives that $subjective$ expectation of $T$ at time $\tau$ is $E_{t}^{s}[T_{\tau}]=-\underbrace{\frac {r}{R}B_{t}}_\text{current deficit}+\textcolor{red}{m_{y}\bar m^{\tau-t}}\underbrace{(d_{\tau}-r \sum_{u=t}^{\tau-1}d_{u})}_\text{future deficit}$
